...
const LoaderCss: WebpackBuilderMiddleware = (config: WebpackConfig, builder: WebpackBuilder): WebpackConfig => {
	...
	const newConfig: any = merge(config, {
		...
		plugins: removeNil([
			...
			builder.ifDevClient(() =>
				new HappyPack({
					...
					threads: 4,
					loaders: [
						builder.useOwn('style-loader'),
						{
							path: builder.useOwn('css-loader'),
		...
		// 								{
		// 									loader: builder.useOwn('happypack/loader'),
		...
		// 						builder.ifNode({
		// 							loaders: [builder.useOwn('css-loader/locals')],
		// 						}),
		// 					),
		// 				),
