...
export class FileRegistry extends Registry<ConfigFileInfo> {
	...
	public registerHooks = async (ctx: FileManager): Promise<void> => {
		...
		for (const fileInfo of fileInfoObjects) {
			...
			const hookName = `${ctx.command.id}.${fileInfo.slug}`;
			...
			for(const hookFile of hookFiles) {
				...
				ctx.Hooks.tap(hookName, hookFile, hook);
			}
		}
