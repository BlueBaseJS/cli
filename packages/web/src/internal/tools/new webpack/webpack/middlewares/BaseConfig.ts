 ...
 * @param builder 
 */
const BaseConfig: WebpackBuilderMiddleware = (config: WebpackConfig, builder: WebpackBuilder): WebpackConfig => {

	const resolve = (!builder.bundleConfig.extensions) ? {} :
		{ extensions: builder.bundleConfig.extensions.map((ext: string) => `.${ext}`) };
	...
	return merge(config, {
		...
		target: builder.bundleConfig.target || builder.isClient
		...
		entry: {
			...
			index: [
				...
				builder.bundleConfig.srcEntryFile,
			],
		},
		...
		output: {
			...
			publicPath: builder.bundleConfig.publicPath,
			...
			// 	`http://${builder.configs.host}:${builder.bundleConfig.devServerPort}${builder.bundleConfig.publicPath}`,

			// 	// Otherwise we expect our bundled client to be served from this path.
			// 	builder.bundleConfig.publicPath,
			// ),
		},
		...
		resolve: {
			...
			// extensions: builder.bundleConfig.bundleSrcTypes.map((ext: string) => `.${ext}`),
		...
		optimization: {
			minimizer: builder.ifProdClient([
				new UglifyJsPlugin({
					...
					sourceMap: builder.bundleConfig.includeSourceMapsForOptimisedClientBundle,
		...
		plugins: removeNil([
			...
			builder.ifDev(() => new webpack.NoEmitOnErrorsPlugin()),
		]),
			...
			rules: [{
				oneOf: [
					...
					ifElse(builder.isClient || builder.isServer)(() => ({
						loader: builder.useOwn('file-loader'),
						...
						query: {
							...
							publicPath: builder.isDev
								...
								builder.configs.bundles.client.webPath,
