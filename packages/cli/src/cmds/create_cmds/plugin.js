...
function handler(name) {
	...
	// });
	const args = [name];
	shell.echo(chalk.blue('Creating BlueRain Plugin! 🔌'));
	const execCommand = path.resolve(__dirname, '..', '..', '..', 'node_modules/.bin/bluerain-cli-create-plugin');
...
exports.handler = function(argv) {
	if (argv && argv.name) {
		...
		require('../../wizard/name_wizard')('Plugin').then((name) => {
			handler(name);
		});
	}
};
