...
const GitHubCloner = function () {
	...
	// Closure variables
	const projectPath = _path2.default.resolve(process.cwd());
	let gitHubUrl = '';
	let targetDir = '';
	let projectType = '';
 ...
 */
	function _validateGitUrl(url) {
		return url ? typeof url === 'string' ? _isGitUrl(url) ? true : _error('Please provide valid git url.') : _error('Url must be a string.') : _error('Url should\'t be empty.');
	}
	/**
  ...
  */
	function _validateProjectDir(dir) {
		return !_fs2.default.existsSync(dir) ? true : _error('Project directory already exist.');
	}
  ...
  */
	function _buildCommand(commands) {
		...
		if (!commands.length || commands.length === 1 && typeof commands[commands.length - 1] === 'function') {
			...
			const len = typeof commands[commands.length - 1] === 'function' ? (fn = commands[commands.length - 1], commands.length - 1) : commands.length;
  ...
  */
	function _exectueCommand(...commands) {
		(0, _child_process.spawn)(_buildCommand(commands), { shell: true, stdio: 'inherit' }).on('exit', code => {
	...
	return function ({ url = '', name = 'bluerain-project', type } = {}) {
		...
		gitHubUrl = url;
		targetDir = _path2.default.join(projectPath, name);
		projectType = type;
