...
describe('ifElse', () => {
	...
	it('the condition can be a function', () => {
		const ifTrue = ifElse(() => true);
		const result = ifTrue('true', 'false');
		expect(result).toBe('true');
	});

	it('when true, returns the first argument', () => {
		const ifTrue = ifElse(true);
		const result = ifTrue('true', 'false');
		expect(result).toBe('true');
	});

	it('when false, returns the second argument', () => {
		const ifFalse = ifElse(false);
		const result = ifFalse('true', 'false');
		expect(result).toBe('false');
	});

	it('when true, can call a function if given as parameter', () => {
		const ifTrue = ifElse(true);
	...
	it('when false, can call a function if given as parameter', () => {
		const ifFalse = ifElse(false);
		const functionMock = jest.fn();
