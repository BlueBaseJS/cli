 ...
 * @param builder
 */
const BaseConfig = options => (config, builder) => {
  const resolve = !builder.configs.extensions ? {} : {
    extensions: builder.configs.extensions.map(ext => `.${ext}`)
  };
  const env = options && options.env ? options.env : {};
  return (0, _webpackMerge.default)(config, {
    // Mode
    mode: builder.configs.mode,
    target: builder.configs.target || (builder.isClient ? // Only our client bundle will target the web as a runtime.
    ...
    entry: {
      ...
      index: [// The source entry file for the bundle.
    ...
    output: {
      ...
      filename: builder.ifProdClient( // For our production client bundles we include a hash in the filename.
      ...
      '[name]-[chunkhash].js', // For any other bundle (typically a server/node) bundle we want a
      ...
      publicPath: builder.configs.publicPath // publicPath: builder.ifDev(
    ...
    performance: builder.ifProdClient( // Enable webpack's performance hints for production client builds.
    {
      hints: 'warning'
    }, // Else we have to set a value of "false" if we don't want the feature.
    false),
    resolve: (0, _objectSpread2.default)({}, resolve, {
      // These extensions are tried when resolving a file.
      extensions: builder.configs.extensions.map(ext => `.${ext}`),
      // modules: [useOwn(), 'node_modules'],
      alias: {
        '@bluebase/core': _cliCore.Utils.fromProjectRoot('node_modules/@bluebase/core'),
    ...
    optimization: {
      minimizer: builder.ifProdClient([new _uglifyjsWebpackPlugin.default({
        // Enable file caching
        cache: true,
    ...
    plugins: removeNil([// These are process.env flags that you can use in your code in order to
    ...
    new _webpack.EnvironmentPlugin((0, _objectSpread2.default)({}, env, {
      ...
      NODE_ENV: builder.isProd ? 'production' : 'development' // SERVER_CONFIGS: ifNode(() => JSON.stringify(configs))

    })), // We don't want webpack errors to occur during development as it will
    // kill our dev servers.
    builder.ifDev(() => new _webpack.NoEmitOnErrorsPlugin()), // Stylish build output on console
    builder.ifDev(() => new WebpackStylish())]),
    module: {
      // Use strict export presence so that a missing export becomes a compile error.
      strictExportPresence: true,
      rules: [{
        oneOf: [// ASSETS (Images/Fonts/etc)
        ...
        ifElse(builder.isClient || builder.isServer)(() => {
          return {
            ...
            loader: (0, _useOwn.useOwn)('file-loader'),
            query: {
