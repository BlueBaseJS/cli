...
describe('filterWithRules', () => {
	test('throws an exception if rules argument is not an object', () => {
		...
		const obj = { a: 1, b: 2 };
		expect(() => filterWithRules(rules, obj)).toThrowError('Filter set an "allow" on path "0", however, this path was not found on the source object.', );
	});

	test('throws an exception if the object to filter is not an object', () => {
		const filterRules = { a: true };
		const obj = 'something else';
		expect(() => filterWithRules(filterRules, obj)).toThrowError('Filter set an "allow" on path "a", however, this path was not found on the source object.', );
	});
